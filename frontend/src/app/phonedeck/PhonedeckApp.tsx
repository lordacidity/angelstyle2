...
export function PhonedeckApp() {
  ...
  const uploadOne = (file: File) => {
    ...
    xhr.open("POST", `${PHONEDECK_URL}/api/upload`);
  ...
  useEffect(() => {
    const fetchPrefs = async () => {
      try {
        const s = await fetch(`${PHONEDECK_URL}/api/settings`).then((r) => r.json());
        notifyPrefs.current = {
  ...
  const remove = async (fileName: string) => {
    ...
    await fetch(`${PHONEDECK_URL}/api/files/${encodeURIComponent(fileName)}`, { method: "DELETE" });
  };

  const launchScrcpy = async (serial: string) => {
    await fetch(`${PHONEDECK_URL}/api/scrcpy`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const launchAllScrcpy = async () => {
    ...
    setOpeningAll(true);
    try {
      await fetch(`${PHONEDECK_URL}/api/scrcpy/all`, { method: "POST" });
    } finally {
  ...
  const saveFile = async (fileName: string) => {
    await fetch(`${PHONEDECK_URL}/api/files/${encodeURIComponent(fileName)}/save`, { method: "POST" });
  };

  const backlogFile = async (fileName: string) => {
    await fetch(`${PHONEDECK_URL}/api/files/${encodeURIComponent(fileName)}/backlog`, { method: "POST" });
  };
