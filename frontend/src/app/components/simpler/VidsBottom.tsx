...
// lib/simpler/vidsBottom for how they are read), so the choice here is who, from
...
import type { BottomPerson, Direction } from '@/lib/simpler/vidsBottom';
...
/** Questions are typed into a search bar, and every one of them is written the
 *  way that bar is: lower case throughout, whoever wrote it. */
...
export function VidsBottomPopup({ people, current, onGo, onClose }: {
  ...
  const [question, setQuestion] = useState(() => {
    try { return lower(localStorage.getItem(QUESTION_KEY) ?? ''); } catch { return ''; }
  });
  ...
  // pressing the other one. Neither button is anything until it is used —
  // the one whose question is actually in the box is the one that colours up,
  // so it says what is there rather than what was last clicked. Edit the words
  // and it stands down again, the question no longer being its.
  ...
  const chosenKind = written && written.text === question ? written.kind : null;
  const writeRef = useRef<AbortController | null>(null);
  // A question still on its way when the popup closes belongs to nobody.
  useEffect(() => () => writeRef.current?.abort(), []);

  const suggest = async (kind: QuestionKind) => {
    if (!person || writingKind) return;
    ...
    try {
      ...
      const { question: q } = await writeQuestion({ person: person.label, kind }, ctrl.signal);
      if (ctrl.signal.aborted) return;
      const text = lower(q);
      setQuestion(text);
      setWritten({ kind, text });
      // Straight into the box, so it can be edited without reaching for it.
  ...
  // label lower-cased and squeezed (lib/simpler/vidsBottom personKey), which is what
    ...
    <Shell title="Make the bottom" subtitle="ChatGPT looks them up, then their Pauv clip" onClose={onClose}>
      {people.length === 0 ? (
        ...
        <div className="space-y-3">
          ...
          {/* Not a <label> around the lot: a click on either button would be
              handed to the textarea as well. */}
          <div>
            <label htmlFor="vids-bottom-question" className="mb-1 block text-[10px] uppercase tracking-wide text-zinc-500">
              The question
            </label>
            <textarea
              id="vids-bottom-question"
              ref={questionRef}
              value={question}
              onChange={(e) => setQuestion(lower(e.target.value))}
            ...
            <div className="mt-1.5 flex items-center gap-1.5">
              ...
              {QUESTION_KINDS.map((k) => (
                <button
                  ...
                  disabled={!person || writingKind !== null}
                  title={person ? QUESTION_HINT[k] : 'Choose who first'}
            ...
            {writeError && <p className="mt-1 text-[9px] text-red-400">Couldn&rsquo;t write one: {writeError}</p>}
          </div>
