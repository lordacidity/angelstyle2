...
export function drawMarketRow({
  ...
  if (sparkline && sparkline.length >= 1) {
    ...
    const SPARK_INSET  = Math.round(4 * S);
    const sparkDrawLeft = sparkLeft + SPARK_INSET;
    const sparkDrawW    = SPARK_W - SPARK_INSET * 2;
    ...
    const pts = vals.map((v, i) => ({
      x: sparkDrawLeft + (i / (vals.length - 1)) * sparkDrawW,
      y: vRange === 0
        ? sparkTop + SPARK_H / 2
