...
export function useVideoLoading({
  ...
  useEffect(() => {
    ...
    const handleLoadedMetadata = () => {
      ...
      console.log('[Charts Row ' + (rowNumber + 1) + '] Video loaded successfully:', { videoId, vw, vh });
      if (vw && vh) {
  ...
  useEffect(() => {
    ...
    console.log('[Charts Row ' + (rowNumber + 1) + '] Video source changed:', videoId);
    setVideoError(null);
    ...
    const timeoutId = setTimeout(() => {
      if (video.readyState < 2) {
        console.error('[Charts Row ' + (rowNumber + 1) + '] Video timeout:', videoId);
