...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt = [
      ...
      `  - subheadline: MUST be "".`,
      ``,
      `PAGE 3 — slideType "supporting_1": THE PAUV CARD. This is the payoff — frame the story as a MARKET move on ${personName}'s Pauv market. It must read as Pauv-centric, not as more news.`,
      `  - ${personName}'s NPSI is their cultural attention/sentiment index on Pauv — it rises when a figure is being watched, argued about and searched for, and falls when they go quiet or the sentiment sours.`,
      `  - Make the explicit connection: does this story push ${personName}'s NPSI UP or DOWN, and WHY — or what a trader reading this moment right now would do about it. Take a clear directional stance.`,
      `  - Name the MECHANISM that converts this story into attention (the rewatches, the headlines, the debate, the search spike) and turn it into a market read — the signal traders are pricing in before the rest of the market catches up.`,
      `  - NO numbers on this page. No boilerplate like "all eyes are on them".`,
      `  - headline: ONE SHORT sentence making the market turn, <= 65 chars (e.g. "THIS IS WHAT AN NPSI SPIKE LOOKS LIKE" or "THE MARKET IS ALREADY REPRICING HIM").`,
      `  - subheadline: REQUIRED — one SHORT sentence, <= 65 chars, giving the TRADER takeaway from a different angle (what it means for anyone holding or watching this market).`,
      ``,
      `Copy rules:`,
