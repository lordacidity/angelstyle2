...
// drawn into its region with the same drawInRegion() the live preview uses,
...
export async function composeSequence(opts: ComposeOptions): Promise<Blob> {
  ...
  try {
    ...
    // Open and measure everything first, because the footage decides how the
    // file is written: a build of 60fps clips laid onto a fixed 30 throws half
    // the motion away, and one written at a fixed bitrate throws away detail the
    // sources were carrying. Both are losses nobody asked for.
    ...
    for (const item of plan.items) {
      ...
      const input = new Input({ source: new UrlSource(item.video.url), formats: ALL_FORMATS });
      inputs.push(input);
    ...
    const wantAudio = (audible.length > 0 || wantRoom || wantMusic)
