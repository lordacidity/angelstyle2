...
//              Prep and baked into that clip's file (see lib/simpler/vidsEdit).
...
// ── The BOOM ─────────────────────────────────────────────────────────────────
// The bang a BOOM makes. It is not in the BOOM clip: the picture is one fixed
// file the house owns (BOOM_URL in lib/simpler/vidsBottom) and the sound is chosen per
// BOOM, so welding them together would mean one file per sound and no way to
// change it. They are laid side by side instead — the picture painted onto the
// frame, the sound scheduled underneath at the moment the picture starts.
//
// Which also lets the two run at their own speeds. The picture is sped up to
// land in BOOM_LENGTH whatever the clip runs for; the bang is not touched, so
// a sound effect is heard the length it was made.

/** The folder the BOOM sounds are served from. Anything dropped in here is in
 *  the list next time it is asked for — see /api/vids/boom-sounds. */
export const BOOM_SOUND_DIR = '/audio/booms';

/** One sound a BOOM can make, as the route hands it over. */
export interface BoomSound {
  url: string;
  label: string;
}

/** How loud the BOOMs sit. Their own level rather than the clips’: a BOOM is
 *  punctuation, and it is meant to land above what it lands on. Attenuation
 *  only, like the clips, so the stage preview (an <audio> element, which cannot
 *  go above 1) and the export agree. */
export const MIN_BOOM_LEVEL = 0;
export const MAX_BOOM_LEVEL = 1;
export const DEFAULT_BOOM_LEVEL = 0.9;
export const clampBoomLevel = (v: number): number =>
  (Number.isFinite(v) ? Math.min(MAX_BOOM_LEVEL, Math.max(MIN_BOOM_LEVEL, v)) : DEFAULT_BOOM_LEVEL);
export const boomGain = (level: number): number => clampBoomLevel(level);
...
export async function listBoomSounds(signal?: AbortSignal): Promise<BoomSound[]> {
  ...
  if (!Array.isArray(rows)) return [];
  return rows.filter(
    (t): t is BoomSound => !!t && typeof t.url === 'string' && typeof t.label === 'string',
  );
}
...
/** Lay a sample down once, at `at` output seconds, at its own speed — a hit
 *  rather than a bed. It runs its whole length unless `end` cuts it short,
 *  which is what stops a BOOM near the finish ringing on past the last frame.
 *  Faded at both ends for the reason scheduleLoop is: to hide the click. */
export function scheduleOnce(
  ctx: BaseAudioContext, sample: AudioBuffer, at: number, gain: number, end = Infinity,
): void {
  const stop = Math.min(end, at + sample.duration);
  const len = stop - at;
  if (len <= 0 || gain <= 0) return;
  ...
  g.gain.setValueAtTime(gain, Math.max(at + fade, stop - fade));
