...
import type { BoomSound } from '@/lib/vidsAudio';
 ...
 *  itself and keeps it for that one video (lib/vidsLocal), so footage dropped
...
export function boomPlanItem(
  ...
  const start = clamp(boom.at, 0, Math.max(0, total - span));
  const end = Math.min(total, start + span);
  if (!(end > start) || len <= 0) return null;
