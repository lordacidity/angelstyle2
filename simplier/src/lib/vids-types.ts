...
// search only makes sense before the recording of the thing it found. The
// pairs here were written down by the Build page's Bottom card back when it
// filed the ChatGPT search it rendered; it keeps that recording in the tab now
// (lib/vidsLocal), so nothing writes a new pair — the rows stand for the
// Bottom As already in the library. A Bottom A with no pair is treated as
// unlinked rather than unusable.
...
export interface VidBuildSpec {
  ...
  captions: {
     ...
     *  scaleCaptionStyle). Absent on records written before the size could be
     *  moved — those were all drawn at the look's own size, which is 1. */
    size?: number;
    notes: string;
    emojis: boolean;
  };
