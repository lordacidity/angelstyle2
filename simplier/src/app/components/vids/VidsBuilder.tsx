...
// VidsBuilder — the stacked-sequence preview and the four things anyone here
// decides. Six slots (Start, Top A/B, Bottom A/B, End) each hold one clip from
// the library; buildPlan() turns the picks into a timeline, the canvas previews
// it live from hidden <video> elements, and Download renders the identical
// timeline to an MP4 via lib/vidsCompose (WebCodecs in the browser — no
// server) straight to this PC — always OUT_W×OUT_H, H.264 + AAC, at OUT_FPS.
// The stage is a downscale and plays whatever the clips are; only the file is
// held to that.
//
// There is one sidebar, and four things on it:
//   Persona   fills Start, Top A and Top B in one go.
//   Bottom    fills Bottom A and Bottom B in one go: pick who, from the people
//             there are Bottom B clips of, which way, and the ChatGPT
//             question. Go renders the ChatGPT recording (see VidsBottom) and
//             puts it in Bottom A for this one video — it lives in this tab
//             (lib/vidsLocal), never in the library, and goes when it is
//             replaced or the build is reset — over their Bottom B.
//   Sound     the song under the video, the room tone, how loud the clips sit.
//   Captions  written by the model off the clips' context, editable by hand,
//             and the look — and the size — they are drawn in.
// Under those, the post caption for Instagram — drafted on its own from what
// the screen recordings show, shown as its opening lines with a Copy button —
// and Download MP4.
//
// Everything else about a build still happens; none of it is anybody's to set.
// End fills itself from its folder, the frame is always 9:16, there are no
// bars, Bottom A is always on Fast, and every clip sits where its slot's
...
//
// The transport under the stage does one thing besides playing it: Insert boom
// lays the BOOM clip over the top of the whole video. The button arms one and
// the bar says where — press Insert, then press the bar on the moment it
// should land. While it is armed the stage shows the BOOM at the playhead, so
// running along the bar shows where it goes rather than only saying. Press the
// mark it leaves on the bar to take that one off. There is no limit: Insert
// arms another rather than moving the last, and two that overlap stack in the
...
// lib/vidsPlan.
//
// Every export is still written down under a short code (lib/vidsRecipe → the
// recipes API): a three-word title from the model, plus the persona, every
// slot's clip and settings, sound and captions. The code goes after the title
// in the file name and on the end of the post caption, so a video found later
// can be traced back to the build that made it.
...
import { PERSONA_PARTS, isPhoto } from '@/lib/vids-types';
import { isLocalClip, makeLocalClip, releaseLocalClip } from '@/lib/vidsLocal';
import {
  DEFAULT_BARS, DEFAULT_BOTTOM_A_PACE, PERSONA_PART_SLOT, SLOTS,
  buildPlan, boomIdOf, boomLayer, boomPlanItem, drawPlanItem, freshPick, isBoomItem, slotSpeed, smoothScaling,
  type BoomInsert, type LayerId, type Picks, type Plan, type PlanItem, type Rect, type SlotId, type SlotPick,
} from '@/lib/vidsPlan';
import { VidsPersonaPicker } from './VidsPicker';
import { VidsBottomPopup } from './VidsBottom';
import {
  HOUSE_BOOM, boomFromBottomB, bottomAContext, bottomAMarks, bottomAName, parseBottomB, peopleFromBottomB, personKey,
...
import { VidsPostCaption } from './VidsPostCaption';
import { createRecipe, deleteRecipe, writeCaptions } from '@/lib/vids-client';
import { specFromBuild } from '@/lib/vidsRecipe';
import {
...
// ── The two clip choices ──────────────────────────────────────────────────────

/** One of the two choices there are to make: a picture, and one word under it.
 *  Empty, that word is what the card is for ("Persona"); filled, it is what is
 *  in it. Nothing else — no hint, no Choose, no clear. The whole tile is the
 *  button. */
function ChoiceCard({ label, name, thumbUrl, disabled, onChoose }: {
  label: string;
  /** What is in the slot, or null while nothing is — then the label stands in. */
  name: string | null;
  ...
  return (
    <button
      ...
      className={`flex min-w-0 flex-1 flex-col items-center gap-1.5 rounded-md border p-2 transition-colors ${
    ...
    >
      {/* A band rather than a square: these sit at the very top of the
          sidebar and everything real is below them. */}
      <div className="flex h-12 w-full items-center justify-center overflow-hidden rounded bg-black">
        {thumbUrl ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={thumbUrl} alt="" className="h-full w-full object-cover" draggable={false} />
        ) : (
          <VideoIcon size={16} className="text-zinc-700" />
        )}
      </div>
      <p
        className={`w-full truncate text-center text-[11px] ${name ? 'text-zinc-100' : 'text-zinc-400'}`}
        title={name ?? label}
      >
        {name ?? label}
      </p>
...
interface Props {
  ...
  /** Look a library clip up by id — what a persona's three parts resolve
   *  through. */
  resolveVideo: (id: string) => VidRow | undefined;
  /** The clips filed under a slot's folder — where End and the list of people
   *  with Bottom B clips come from. */
  ...
  active: boolean;
  /** The library has arrived — until it has, there is nothing to choose from. */
  libraryLoaded: boolean;
}

export function VidsBuilder({
  picks, onPicksChange, resolveVideo, clipsForSlot,
  personas, appliedPersonaId, onUsePersona, active, libraryLoaded,
}: Props) {
  const outW = OUT_W;
  const outH = OUT_H;
  const bars = BARS;
  ...
  /** Which of the two choosers is open. */
  const [picker, setPicker] = useState<{ kind: 'persona' } | { kind: 'bottom' } | null>(null);
  ...
  // The Bottom card's recording lives in this tab only (lib/vidsLocal), and
  // its bytes go when it does: replaced by the next one (chooseBottom), the
  // build reset, or the page left. A library clip passes through untouched.
  const dropLocal = (p: SlotPick | undefined) => { if (p && isLocalClip(p.video)) releaseLocalClip(p.video); };
  useEffect(() => () => dropLocal(picksRef.current.bottomA), []);
  ...
  // recordings — that one wins, and is kept out of the people there are
  // recordings of, being nobody to trade on (see lib/vidsBottom). Either way
  // there is always one, so the button is never waiting on a clip.
   ...
   *  Bottom A, theirs for Bottom B. An End is filled in too if the stage
   *  hasn't got one; then the captions, once the clips have loaded — the
   *  sidebar says so while they do. Lands on the stage as it is NOW, not as
   *  it was when the job was started. The recording it replaces, if that was
   *  one of ours, is let go. */
  const chooseBottom = (a: VidRow, b: VidRow) => {
    ...
    timeRef.current = 0;
    setTime(0);
    if (now.bottomA?.video.id !== a.id) dropLocal(now.bottomA);
    onPicksChange(() => next);
   ...
   *  the model answers with them on top, the recording renders, and it is
   *  held here for this video (lib/vidsLocal) with its three beats as marks —
   *  so the captions go search this, wait for it, pick them, each on its
   *  moment — and both clips land on the stage (chooseBottom). Nothing goes
   *  to the library. A Go while one is running drops that one. */
  const startBottomJob = (person: BottomPerson, direction: BottomDirection, question: string) => {
    ...
    void (async () => {
      try {
        const { blob, reply, beats, seconds, width, height, poster } = await makeChatGptClip({
          name: person.label,
          direction,
          question,
          signal: ctrl.signal,
        ...
        const row = makeLocalClip(blob, {
          name: bottomAName(person.label, direction),
          context: bottomAContext(question, answer),
          marks: bottomAMarks(beats, question, answer),
          duration: seconds,
          width,
          height,
  ...
  /** The persona. The hook is the persona's line, so choosing one over a vid
   *  that is already on the stage has the captions written again around them.
   *  With no vid yet there is nothing to write about — choosing one will do
   *  it. */
  const choosePersona = (persona: VidPersona) => {
   ...
   *  picks for itself anyway. The sound levels are left alone — those are how
   *  you work, not what you picked. Nothing leaves the library; this clears
   *  the stage, it doesn't delete footage. */
  const resetBuild = () => {
    pause();
    setPicker(null);
    const end = rollEnd();
    dropLocal(picks.bottomA);
    onPicksChange(() => (end ? { end } : {}));
    setBooms([]);
    setBoomArming(false);
    setBoomLevel(DEFAULT_BOOM_LEVEL);
    rollMusic();
    rollStyle();
  ...
  const working = busy || writing || !!bottomJob;
  /** Whether anybody has chosen anything yet. End fills itself, so a stage
   *  with only that on it has not been started. */
  const started = !!appliedPersonaId || !!picks.bottomA || !!bottomJob;
  const status = (() => {
    if (bottomJob) {
    ...
    if (working) return 'Putting it together — the rest of the clips, the sound and the captions.';
    // Nothing chosen yet: the two tiles say what to do, so this says nothing.
    if (!appliedPersonaId && !picks.bottomA) return '';
    if (!appliedPersonaId) return 'Now choose a persona.';
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1">
      ...
      {/* And a bang per BOOM that has one. Not muxed into the clip: the sound
          ...
          side by side — see the BOOM section in lib/vidsAudio. Left at its own
          ...
          >
            ...
            {total > 0 && boomItems.map((item) => (
              ...
              <button
                ...
                title={`BOOM at ${fmtTime(item.start)} — press to take it off`}
              ...
              >
                <span className="h-[18px] w-[3px] rounded-full bg-amber-400 transition-all group-hover:h-[22px] group-hover:w-[5px] group-hover:bg-white" />
              </button>
            ))}
          </div>
      ...
      <aside className="flex w-[320px] shrink-0 flex-col overflow-y-auto border-l border-zinc-800">
        <div className="border-b border-zinc-800 px-3 py-3">
          <div className="mb-2 flex items-center gap-2">
            <p className="min-w-0 flex-1 truncate text-[12px] font-semibold text-zinc-200">Build some goyslop</p>
            <button
              onClick={resetBuild}
              disabled={working || !!exporting || (!filled.length && !hasLines)}
              title="Clear the whole thing — both clips, the captions written for them — and start again on a fresh End and a fresh song. The library is untouched."
          ...
          <div className="flex items-start gap-2">
            <ChoiceCard
              ...
              thumbUrl={picks.topA?.video.thumbUrl ?? picks.start?.video.thumbUrl ?? null}
            ...
            <ChoiceCard
              label="Bottom"
              name={bottomJob
                ? `${bottomJob.person} · ${bottomJob.direction}…`
                : picks.bottomB
          ...
          {status && (
            <div className="mt-2 flex items-start gap-1.5 text-[10px] text-zinc-500">
              ...
              {bottomJob && (
                <button onClick={cancelBottomJob} title="Drop it" className="shrink-0 text-zinc-500 hover:text-red-400">Cancel</button>
              )}
            </div>
          )}
        </div>
