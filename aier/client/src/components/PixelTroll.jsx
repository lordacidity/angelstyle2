...
import { getGenerating } from '../trollBus.js';
 ...
 *
 * He can also be driven from the keyboard: a handful of letter keys are each
 * wired to a behavior. Pressing one lets the current action finish, then
 * seamlessly switches to the requested one. (Which letters? Go find out.)
 *
...
// keyboard map: each letter drives one behavior (intentionally undocumented).
const KEYMAP = {
  b: 'walk', f: 'run', j: 'jump', v: 'wave', s: 'sit', x: 'stretch', l: 'look',
  q: 'backflip', e: 'splits', r: 'cartwheel', z: 'sleep', w: 'shake', g: 'giant',
  m: 'moonwalk', d: 'disco', o: 'roll', h: 'dig', n: 'headbang', t: 'kick',
  p: 'trampoline', k: 'breakdance', c: 'faint', a: 'sneeze',
};

// Shift + letter forces an ultra-rare cameo on demand (the random ~1% triggers
// still fire on their own). s = smitten, b = bloodDeath, u = ufo.
const SHIFT_KEYMAP = { s: 'smitten', b: 'bloodDeath', u: 'ufo', d: 'dog', x: 'stomp' };
...
export default function PixelTroll({ hidden = false }) {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      canvas.height = STAND_H * 3 + 60; // headroom for giant + jumps
    };
    resize();
    ...
    // keyboard control — finish current action, then seamlessly switch.
    function onKey(e) {
      if (!getGenerating() || hiddenRef.current) return; // keys only while generating
      if (e.metaKey || e.ctrlKey || e.altKey) return;
      const el = document.activeElement;
      if (el && /^(INPUT|TEXTAREA|SELECT)$/.test(el.tagName)) return;
      const key = e.key.toLowerCase();
      const action = e.shiftKey ? SHIFT_KEYMAP[key] : KEYMAP[key]; // Shift = force a cameo
      if (!action) return;
      e.preventDefault();
      T.userQueue.push(action);
      const now = performance.now();
      if (T.mode === 'away') {
        T.mode = 'active'; T.x = W() * rand(0.3, 0.7); T.yOffset = 0; T.scale = 1; T.mirror = false;
        beginNext(now); // appear and run it immediately
      } else if (T.action === 'idle') {
        beginNext(now); // idle is interruptible, switch right away
      }
      // otherwise: current action runs to completion, then beginNext picks it up
    }
  ...
  return (
    <canvas
      ref={canvasRef}
      style={{
        position: 'fixed', left: 0, bottom: 0, width: '100vw', zIndex: 60,
