...
- **The Vercel `/ai-maker` page drives THIS local server.** Its SPA (in the main `frontend/`
  app) calls `http://localhost:3010` cross-origin for the whole studio backend — it's launched
  by the user via the `/ai-maker` "Launch Aier server" button (`aier://` → `launch-aier.bat` →
  `npm run dev` with `AIER_UNGATED=1`). `server/index.js` accepts the SPA's `/api/aier/*` paths
  (strips the `aier` segment), adds CORS + `Access-Control-Allow-Private-Network`, and `api.js`
  absolutizes `/media/...` URLs to `localhost:3010`. The USER starts it, never Claude.
